"""
Swiss Roll Decision Boundary Analysis with Poisoning

This module contains tools for:
1. Generating synthetic spiral datasets (with optional poisoning)
2. Visualizing model decision boundaries
3. Analyzing model sensitivity to parameter perturbations
4. Evaluating poisoning effects on model performance

Key Features:
- Two-spiral dataset generation with configurable noise
- Decision boundary visualization for binary classifiers
- Model perturbation analysis ("wiggle test")
- Poisoning attack simulation and evaluation
- Animation of decision boundary evolution
"""
...
# Note: IPython.display is only needed for notebook environments
...
def two_spirals(n_points, noise=0.5, seed=None):
    """
    Generate two interlocking spirals dataset with Gaussian noise.
    
    Args:
        n_points: Number of points per spiral (total points = 2*n_points)
        noise: Standard deviation of Gaussian noise added to points
        seed: Random seed for reproducibility
        
    Returns:
        points: Tensor of shape [2*n_points, 2] containing (x,y) coordinates
        labels: Tensor of shape [2*n_points] containing binary labels (0 or 1)
    """
    gen = torch.Generator()
    if seed is not None:
        gen.manual_seed(seed)

    n_rotations = 1.7  # Number of full rotations for the spiral
...
def visualize_poisoned_results_separate(
    all_models,
    ...
    for i, model_data in enumerate(all_models):
        ...
        plt.plot(model_data['train_losses'], 
                 color=colors[i], 
                 label=f"Train ({label_suffix})")
        plt.plot(model_data['test_losses'], 
                 '--', color=colors[i],
    ...
    for i, (additional_data, model_data) in enumerate(zip(dataset_quantities, all_models)):
        ...
        plt.title(f"{dataset_type} ({additional_data} samples)\n"
                  f"Final Test Accuracy: {final_test_acc:.2%}", pad=20)
