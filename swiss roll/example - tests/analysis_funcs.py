...
def plot_fixed_landscape_minima_pair(
    ...
    Can optionally plot the average ± standard deviation across runs.
    ...
    Args:
        ...
        plot_average (bool): If True, plot a thicker line showing the average ± std across runs.
    ...
    if title is None:
        if ranking:
            title = f"Ranks of {ylabel} vs {xlabel}"
        else:
    ...
    if plot_average and processed_x:
        ...
        plt.errorbar(
            ...
            fmt='-o', color='red', linewidth=2, label='Average ± Std'
        )
        plt.legend()

    # Axis labels with custom font sizes
    ...
    if plot_average:
        return x_ref, avg_y, x_std, y_std
...
def plot_minima_volume_vs_data_level(
    ...
    show_plot = True,
    xlabel_size=12, ylabel_size=12, title_size=14,
    data_type=None,              # NEW (optional)
    base_train_size=None         # NEW (optional)
):
    """
    ...
    for target_level, exp_results in results_dict.items():
        ...
        if not plot_only_average:
            for exp_data in exp_results:
                x_vals = exp_data["data_levels"]
                y_vals = exp_data["log_exp"]
                ...
                if target_level in x_vals:
                    idx = x_vals.index(target_level)
                    plt.scatter(
                        target_level, y_vals[idx],
                        color=color,
                        s=120,
                        edgecolors="black",
                        linewidths=1.0,
        ...
        if plot_average:
            ...
            if target_level in data_levels:
                idx = data_levels.index(target_level)
                plt.scatter(
                    target_level, mean_y[idx],
                    color=color,
                    s=140,
                    edgecolors="black",
                    linewidths=1.0,
