...
def verify_model_results(
    ...
    for i, (additional_data, model_data) in enumerate(zip(dataset_quantities, all_models)):
        ...
        plt.scatter(x_base_train[:,0].cpu(), x_base_train[:,1].cpu(),
                    ...
                    s=60, alpha=0.9, label='Base Dataset')
        if additional_data > 0:
            plt.scatter(x_additional[:additional_data,0].cpu(),
                        ...
                        label=f'Additional {dataset_type}')

        # Title / annotation
        final_test_acc = model_data['test_accs'][-1]
        plt.title(
            f"Base Dataset: {len(y_base_train):,} Examples\n"
            f"Additional {dataset_type}: {additional_data:,} Examples\n"
